...

{
 ...
 if(lflag == 0  && tflag == 0 && Sflag == 0 && Rflag == 0) // ls without any flag, default ls in current directory
  ...
  n = scandir(".",&namelist,NULL,alphasort);
   ...
   if(n<0)
     ...
     return -1;
	   ...
	   {
           printf("%s%s%s  ",KBLU,namelist[i]->d_name,KWHT);
	   
	   }
           else
            printf("%s%s   ",KWHT,namelist[i]->d_name);
         free(namelist[i]);
         i++;
        }
printf("\n");
